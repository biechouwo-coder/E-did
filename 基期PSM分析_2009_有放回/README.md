...
本文件夹包含基于2009年基期的倾向得分匹配(PSM)分析代码，使用**有放回**匹配方法。
...
- `控制组使用情况.xlsx`: 控制组城市被使用的次数统计（有放回匹配特有）
...
4. **执行匹配**: 对每个处理组城市,在控制组中寻找倾向得分最接近且差异在卡尺范围内的城市（**控制组可被多次使用**）
...
- **更高的匹配成功率**: 每个处理组都能找到最优匹配
- **更低的偏差**: 不受控制组数量限制，可以找到更接近的匹配
- **效率更高**: 适用于控制组相对较少的情况

需要注意的是:
- 同一个控制组城市可能与多个处理组城市匹配
- 需要在统计分析中考虑权重调整
